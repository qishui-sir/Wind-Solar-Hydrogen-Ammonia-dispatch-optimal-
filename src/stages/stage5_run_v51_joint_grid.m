    ...
    for pacing = pacing_values
        for smoothing = smoothing_values
            ...
            if isfield(config, 'forecast_fallback') && ...
                    ~isempty(config.forecast_fallback)
                rolling_config.forecast_fallback = config.forecast_fallback;
            end
            cached = false;
            if resume
                candidate_path = candidate_rolling_path( ...
                    rolling_output_dir, source_year_value, reserve, ...
                    pacing, smoothing, gate_mode, soft_reserve);
                if isfile(candidate_path)
                    try
                        loaded = load(candidate_path, ...
                            'result', 'run_info', 'metrics');
                        if isfield(loaded, 'result') && ...
                                isfield(loaded, 'run_info') && ...
                                isfield(loaded, 'metrics') && ...
                                isfield(loaded.run_info, 'status') && ...
                                string(loaded.run_info.status) == "completed"
                            candidate = completed_row(candidate, ...
                                loaded.result, loaded.run_info, ...
                                loaded.metrics);
                            cached = true;
                        end
                    catch
...
function path = candidate_rolling_path(output_dir, data_year, reserve, ...
        pacing, smoothing, gate_mode, soft_reserve)
tag = "v51_contract_plan_and_hb_smoothing";
if reserve.quantile > 0
    tag = sprintf('%s_h2q%03d_l%d', tag, ...
        round(100 * reserve.quantile), reserve.lookahead_days);
    if soft_reserve
        tag = sprintf('%s_soft', tag);
    end
end
if pacing > 0
    tag = sprintf('%s_cp%03d', tag, round(100 * pacing));
end
if smoothing > 0
    tag = sprintf('%s_hb%03d', tag, round(100 * smoothing));
end
if gate_mode ~= "off"
